...
(function () {
  ...
  /* -----------------------------------------------------------
     ...
     ----------------------------------------------------------- */
  var CONFIG = {
    // Endpoint que recebe o POST do formulário de candidatura.
    ...
    contactEmail: "experience@auvp.com.br",
  };
  ...
  function preloader() {
    ...
    var count = $(".preloader__count", el);
    var mark = $(".preloader__mark", el);
  ...
  /* -----------------------------------------------------------
     ...
     ----------------------------------------------------------- */
  function reveals() {
    var targets = $$("[data-reveal], [data-split], .media-mask");
  ...
  function navigation() {
    ...
    var onScroll = function () {
      ...
      if (!document.body.classList.contains("menu-open")) {
        nav.classList.toggle("is-hidden", y > 400 && y > last + 4);
      }
      last = y;
    };
    ...
    var setMenu = function (open) {
      ...
      $(".burger__text", burger).textContent = open ? "Fechar" : "Menu";
    ...
    $$(".menu__link, .menu a", menu).forEach(function (a) {
  ...
  function cursor() {
    ...
    var hoverSel = "a, button, input, select, textarea, .chip, [data-cursor]";
  ...
  function parallax() {
    ...
    var items = $$("[data-parallax]").map(function (el) {
      return { el: el, f: parseFloat(el.getAttribute("data-parallax")) || 0.12, y: 0 };
    });
  ...
  function progress() {
    ...
    var nav = $(".dotnav");
    var items = nav ? $$(".dotnav__item", nav) : [];
    ...
    loopAdd(function () {
      ...
      if (!nav) return;
      nav.classList.toggle("is-visible", scroll.y > window.innerHeight * 0.7);
  ...
  function accordion() {
    $$("[data-acc]").forEach(function (root) {
      ...
      $$(".acc__trigger", root).forEach(function (btn) {
        btn.addEventListener("click", function () {
          var item = btn.closest(".acc__item");
          var open = item.classList.contains("is-open");

          if (single) {
            $$(".acc__item", root).forEach(function (o) {
  ...
  function careTabs() {
    ...
    var items = $$(".care__item", list);
    var plates = $$(".care__plate");
    ...
    items.forEach(function (it, i) {
      it.addEventListener("click", function () {
        activate(items.indexOf(it) === indexOfActive() ? -1 : i);
      });
      if (finePointer) {
  ...
  /* -----------------------------------------------------------
     ...
     ----------------------------------------------------------- */
  function dragScroll() {
    $$(".hscroll").forEach(function (root) {
      var track = $(".hscroll__track", root);
      var hint = $(".hscroll__hint span", root.parentNode) || $(".hscroll__hint span");
      if (!track) return;
  ...
  function destinationVote() {
    ...
    toggles.forEach(function (t) {
      t.addEventListener("click", function (e) {
        e.preventDefault();
        if (panel.classList.contains("is-open")) reveal();
        else open();
      });
    });

    $$(".chip", panel).forEach(function (chip) {
    ...
    function syncHidden() {
      var hidden = $("#vote-selected");
      if (!hidden) return;
      hidden.value = $$(".chip.is-on", panel).map(function (c) { return c.textContent.trim(); }).join(", ");
    }
  }
    ...
    $$("form[data-form]").forEach(function (form) {
      ...
      form.addEventListener("submit", function (e) {
        ...
        fetch(CONFIG.formEndpoint, {
          ...
          .catch(function () {
            finish("Não conseguimos enviar agora. Tente novamente ou escreva para " + CONFIG.contactEmail + ".", false);
          });
      });
  ...
  function boot() {
    ...
    preloader().then(function () {
      ...
      $$(".hero [data-split], .hero [data-reveal]").forEach(function (el) { el.classList.add("is-in"); });
    });
  }
